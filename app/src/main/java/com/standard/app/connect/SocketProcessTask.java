package com.standard.app.connect;
...
public class SocketProcessTask extends AsyncTask<byte[], Integer, Void> {
    ...
    protected void onPreExecute() {
        ...
        Log.i("CONTACTING SERVER", "STARTING");
        super.onPreExecute();
    }
    ...
    protected Void doInBackground(byte[]... params) {
        ...
        Log.i("CONTACTING SERVER", "DOING BACKGROUND");
        if (mProcType != PacketProcessUtils.PACKET_PROCESS_CONSUME && mProcType != PacketProcessUtils.PACKET_PROCESS_SCAN) {
            ...
            if (mHostIp == null || mHostPort == null) {
                ...
                if (mHostIp == null || mHostPort == null) {
                    Log.i(TAG, "ip or port is null");
                    mErrorReson = PacketProcessUtils.SOCKET_PROC_ERROR_REASON_IP_PORT;
                    return null;
                }
            }
            Log.i(TAG, "ip and port is ok. IP: " + mHostIp + " PORT: " + mHostPort);

            while (mSendNum < RETRY_MAX_CNT) {
                Log.i(TAG, "mSendNum:" + mSendNum);
                if (!mCommSocket.open(mHostIp, mHostPort)) {
                    Log.i(TAG, mSendNum + " open failed");
                    mSendNum++;
                } else {
                    Log.i(TAG, mSendNum + " open success");
                    break;
                }
            }
            if (mSendNum == 3) {
                Log.i(TAG, "socket open failed.");
                mErrorReson = PacketProcessUtils.SOCKET_PROC_ERROR_REASON_CONNE;
                return null;
            }
            Log.i(TAG, "socket open success.");
...
//        }
            ...
            if ((mRecePacket == null) || (mRecePacket.length <= 0)) {
                Log.i(TAG, "receive packet failed.");
