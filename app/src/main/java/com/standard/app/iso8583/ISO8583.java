package com.standard.app.iso8583;
...
public class ISO8583 {
    ...
    private class ISO8583Domain { /*8583每个域的定义*/
        ...
        public byte mType; /* bit0,bit1暂留，bit2:为0时左对齐,为1时右对齐，bit3:为0时BCD码,为1时ASC码，type:0,1,2,3三种*/
        ...
        public void setDomainProperty(int length, byte type, byte flag, String domainName) {
            ...
            Log.i(TAG, "FROM ISO8583: DOMAINNAME :" + domainName+", LENGTH :"+length+", TYPE :"+type+", FLAG : "+flag);
        }
    }
    ...
    public int strtoiso(byte[] src) {
        ...
        for (int i = 0; i < bitnum; i++) {//位图64位，8个字节，每个字节8位按顺序检测是否为1
            ...
            for (int j = 0; j < bitnum; j++, bitmask>>=1) {//bitmask = 0x80，10000000，每次右移一位，代表判断这个字节中的下一位是否为1
				...
				// String(buf));
				// else
                //HAPAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
                Log.i(TAG, "idx=" + (n + 1) + ", " + mISO8583Domain[n].mDomainName + ", Len=" + len + ",[BCD] " + BCDASCII.bytesToHexString(buf));


                if(n + 1 == 22){
                    posEntryMode_DE22 = BCDASCII.bytesToHexString(buf);
                }else if(n + 1 == 25){
                    posConditionCode_DE25 = BCDASCII.bytesToHexString(buf);
                }else if(n + 1 == 11){
                    systemsTraceAuditNumber_DE11 = BCDASCII.bytesToHexString(buf);
                }else if(n + 1 == 26){
                    posPINCaptureCode_DE26 = BCDASCII.bytesToHexString(buf);
                }else if(n + 1 == 35){
                    track2Data_DE35 = BCDASCII.bytesToHexString(buf);
                }else if(n + 1 == 37){
                    retrievalReferenceNumber_DE37 = BCDASCII.bytesToHexString(buf);
                }else if(n + 1 == 14){
                    Static.dateExpiration_DE14 = BCDASCII.bytesToHexString(buf);
                }else if(n + 1 == 55){
                    Static.iccDATA_DE55 = BCDASCII.bytesToHexString(buf);
                }else if(n + 1 == 39){
                    Static.responseCode_DE39 = BCDASCII.bytesToHexString(buf);
                }else if(n + 1 == 52){
                    Static.pinData_DE52 = BCDASCII.bytesToHexString(buf);
                }else if(n + 1 == 55){
                    Static.iccResponceData_DE55 = BCDASCII.bytesToHexString(buf);
                }else if(n + 1 == 23){
                    Static.cardSequenceNumber_DE23 = BCDASCII.bytesToHexString(buf);
                }else if(n + 1 == 123){
                    Static.posDataCode_DE123 = BCDASCII.bytesToHexString(buf);
                }
